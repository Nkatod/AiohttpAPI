...
async def test_get_items(client):
    # authorize
    token = await authorization(client, f'user1', f'user1')
    # get user items
    data = {'token': token, 'attributes': 'TEST'}
...
async def get_reference_for_move(client, login_sender, login_accepter):
    ...
    # create new item
    item_id = await create_new_item(client, token)
    return await create_reference_to_move(client, token, item_id, login_accepter)

async def test_create_reference_to_move(client):
    reference_to_move = await get_reference_for_move(client, 'user1', 'user2')
...
async def test_get_item_from_another_user(client):
    ...
    reference_to_move = await get_reference_for_move(client, login_sender, login_accepter)
